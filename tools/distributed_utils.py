        ...
        for obj in iterable:
            ...
            if i % print_freq == 0 or i == len(iterable) - 1:
                ...
                for key, val in self.meters.items():
                    tensor_data = torch.tensor(list(val.deque), dtype=torch.float32)
                    # 计算最后 print_freq 个数据的平均值
                    last_values = tensor_data[-print_freq:]
                    average_last_values = last_values.mean().item()
                    if 'lr' in key:
                        # self.writer.add_scalar(tag=key, scalar_value=average_last_values,
                        #                        global_step=(i + (epoch - 1) * len(iterable)))
                        continue
                    if len(tensor_data) < print_freq:
                        continue
                    if self.writer is not None:
                        self.writer.add_scalar(tag=str('train/' + key), scalar_value=average_last_values, global_step=(i + (epoch - 1) * len(iterable)))
            i += 1
            end = time.time()
...
def warmup_lr_scheduler(optimizer, warmup_iters, warmup_factor):
    ...
    def f_(cur_iters):
        ...
        if cur_iters >= warmup_iters:
            return 1
        k = (1 - float(cur_iters) / warmup_iters) * (1 - warmup_factor)
        # warmup_lr = [_lr * (1 - k) for _lr in base_lr]
        # return warmup_lr
        return 1 - k

    return torch.optim.lr_scheduler.LambdaLR(optimizer, lr_lambda=f_)
